...
> **一句话定位**：把你的 OpenClaw agent 变成一个 **DeepSeek Harness（DSH）执行平面**——真实本地命令（odsh.exec）、桌面控制与截图（odsh.cua / odsh.visual）、DSH worker 派发（odsh.serve）、DSH 风格工具插件、常驻 watcher/scheduler 服务，全部以**原生 OpenClaw 插件**（官方 SDK 契约）提供。无需第二个 LLM、无需 Windows-Node + SSH 桥、无需 Docker——只需 openclaw plugins install clawhub:odsh-native。
...
> **为什么要 --acknowledge-clawhub-risk？** 发布安全扫描把本项目标为 suspicious，因为它本质上是一个**本地执行 + 桌面控制**的高权限插件——含 odsh.exec（无 shell）、桌面截图路径、以及可选的远程 worker 调用。插件自身保持 fail-closed（无 shell、工具白名单、路径遏制）；勾选信任标记前建议快速过一遍源码。
...
## 网页搜索（SERPdive + TinyFish 智能选择）

**ODSH-Native 组合两家免费引擎，按请求智能选择：**
...
**智能选择逻辑**：轻量路由——答案式查询先用 SERPdive；需要原始结果时退回 TinyFish Search；调用方要整页正文（urls[]）时用 TinyFish Fetch。两者低量免费（SERPdive：1000 credits/月含免费 krill 模型；TinyFish：Search 与 Fetch 在 $0 下也免费）。无需 DeepSeek 网页搜索余额。
...
- **默认 fail-closed**：无 shell 的 execFile、工具名白名单、CUA_REMOTE 仅 http(s)、插件加载路径遏制、远程 worker 可选 Bearer 鉴权、不提交真实 token、tests/security.test.mjs 拦截被跟踪的个人标识符与密钥。见 docs/OPERATIONS §安全。
